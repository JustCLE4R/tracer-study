...
function buildDynamicForm(question) {
  ...
  // Logika penambahan * pada kondisi tertentu
  var labelText = question.question; 
  if (question.category === "bekerja" && question.id !== 10 && question.id !== 13) {
      labelText += " *";
  }
  
  var label = $("<label>").addClass("form-label text-secondary").text(labelText);
  colDiv.append(label);
